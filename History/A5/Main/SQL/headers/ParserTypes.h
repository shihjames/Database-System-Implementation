...
public:
	string addToCatalog(string storageDir, MyDB_CatalogPtr addToMe)
	{
		...
		if (!isBPlusTree)
		{
			myTable = make_shared<MyDB_Table>(tableName, storageDir + "/" + tableName + ".bin", mySchema);
		...
		else
		{
			...
			myTable = make_shared<MyDB_Table>(tableName, storageDir + "/" + tableName + ".bin", mySchema, "bplustree", sortAtt);
		}
...
public:
	...
	bool check(MyDB_CatalogPtr myCatalog)
	{
		vector<string> tables;
		myCatalog->getStringList("tables", tables);
		unordered_set<string> tableSet(tables.begin(), tables.end());

		// check whether table exists
		for (auto table : tablesToProcess)
		{
			if (tableSet.find(table.first) == tableSet.end())
			{
				cout << "[ERROR] Table: " << table.first << " not exists." << endl;
				return false;
			}
		}
		...
		{
			// check SELECT with grouping
			unordered_set<string> groupSet;
			for (auto groupingClause : groupingClauses)
			{
				if (!groupingClause->check(myCatalog, tablesToProcess))
				{
					cout << "[ERROR] Incorrect grouping." << endl;
					return false;
				}
				groupSet.insert(groupingClause->toString());
			}

			for (auto value : valuesToSelect)
			{
				if (!value->check(myCatalog, tablesToProcess))
				{
					cout << "[ERROR] Incorrect SELECT." << endl;
					return false;
				}
			}
			for (auto value : valuesToSelect)
			{
				if (value->toString().substr(0, 3) == "avg" || value->toString().substr(0, 3) == "sum")
				{
					continue;
				}
				if (groupSet.find(value->toString()) == groupSet.end())
				{
					cout << "[ERROR] \"SELECT " << value->toString() << "\" does not match with grouping." << endl;
					return false;
				}
				else
				{
		...
		{
			if (!disjunction->check(myCatalog, tablesToProcess))
			{
				cout << "[ERROR] Incorrect disjunctions." << endl;
				return false;
			}
		}

		cout << "[SUCCESS] Valid query." << endl;
		return true;
	}
